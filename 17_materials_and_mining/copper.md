...
- 핵심 제약은 mine lead time, ore grades, smelting/refining capacity이다.
- 선행지표는 announced projects, refined copper balance, treatment charges이다.
- 의존 도메인은 Grid, data centers, EVs, and construction이다.
...
|---|---|---|---|---|
| `copper_demand` | demand | index | higher_is_capacity | use_base |
| `copper_capacity` | capacity | index | higher_is_capacity | block |
| `copper_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(copper_capacity)
finance_score = normalized_inverse(copper_finance)
demand_score = normalized(copper_demand)
...
Baseline formula: `min(mined supply, refined supply, recycling supply) versus electrification demand`.
...
- Base: min(mined supply, refined supply, recycling supply) versus electrification demand에 의해 2035 정상 경로를 산출한다.

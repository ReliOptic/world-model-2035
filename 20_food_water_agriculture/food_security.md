# Food, water, and agriculture: food security
...
- 핵심 제약은 crop yields, inventories, import dependence, conflict exposure이다.
- 선행지표는 stock-to-use ratios, food CPI, harvest forecasts, import bills이다.
- 의존 도메인은 Climate, logistics, energy, and geopolitics이다.
...
|---|---|---|---|---|
| `food_security_demand` | demand | index | higher_is_capacity | use_base |
| `food_security_capacity` | capacity | index | higher_is_capacity | block |
| `food_security_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(food_security_capacity)
finance_score = normalized_inverse(food_security_finance)
demand_score = normalized(food_security_demand)
...
Baseline formula: `min(domestic production, import capacity, affordability, reserve adequacy)`.
...
- Base: min(domestic production, import capacity, affordability, reserve adequacy)에 의해 2035 정상 경로를 산출한다.

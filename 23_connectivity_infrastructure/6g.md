# Connectivity infrastructure: 6G
...
- 핵심 제약은 spectrum, standards, device ecosystem, operator capex이다.
- 선행지표는 3GPP milestones, spectrum auctions, telecom capex, testbeds이다.
- 의존 도메인은 On-device AI, sensors, telecom, and semiconductors이다.
...
|---|---|---|---|---|
| `6g_demand` | demand | index | higher_is_capacity | use_base |
| `6g_capacity` | capacity | index | higher_is_capacity | block |
| `6g_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(6g_capacity)
finance_score = normalized_inverse(6g_finance)
demand_score = normalized(6g_demand)
...
Baseline formula: `min(standards maturity, spectrum availability, operator capex, device readiness)`.
...
- Base: min(standards maturity, spectrum availability, operator capex, device readiness)에 의해 2035 정상 경로를 산출한다.

# Food, water, and agriculture: water stress
...
- 핵심 제약은 basin withdrawals, glacier/snowpack decline, groundwater depletion이다.
- 선행지표는 reservoir levels, drought indices, irrigation restrictions이다.
- 의존 도메인은 Climate, food, energy, cities, and mining이다.
...
|---|---|---|---|---|
| `water_stress_demand` | demand | index | higher_is_capacity | use_base |
| `water_stress_capacity` | capacity | index | higher_is_capacity | block |
| `water_stress_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(water_stress_capacity)
finance_score = normalized_inverse(water_stress_finance)
demand_score = normalized(water_stress_demand)
...
Baseline formula: `min(renewable water availability, storage, demand management, treatment capacity)`.
...
- Base: min(renewable water availability, storage, demand management, treatment capacity)에 의해 2035 정상 경로를 산출한다.

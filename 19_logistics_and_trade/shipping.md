...
- 핵심 제약은 fleet capacity, shipyard slots, canal/chokepoint reliability이다.
- 선행지표는 freight rates, orderbook, port congestion, fuel rules이다.
- 의존 도메인은 Energy, trade, materials, and manufacturing이다.
...
|---|---|---|---|---|
| `shipping_demand` | demand | index | higher_is_capacity | use_base |
| `shipping_capacity` | capacity | index | higher_is_capacity | block |
| `shipping_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(shipping_capacity)
finance_score = normalized_inverse(shipping_finance)
demand_score = normalized(shipping_demand)
...
Baseline formula: `min(fleet capacity, route reliability, fuel compliance, port throughput)`.
...
- Base: min(fleet capacity, route reliability, fuel compliance, port throughput)에 의해 2035 정상 경로를 산출한다.

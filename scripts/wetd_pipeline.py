...
SIGNAL_SELECTION_RATIONALE = [
    (
        "Why these five?",
        "They are the smallest MECE set that separates a war-economy transition from an obvious crisis headline: supply self-reliance, security stockpiling, route rewiring, permissioned access, and civilian capacity being pulled into defense/intelligence lanes.",
    ),
    (
        "How to read a country fingerprint",
        "A high score is a review cue, not a verdict. The inference becomes useful when several independent mechanisms move together inside a theater exposure graph; one causal row by itself is deliberately insufficient.",
    ),
    (
        "What data is underneath",
        "The batch loads public rows into DuckDB: Federal Register policy events, Treasury OFAC sanctions, USAspending procurement, NSIDC Arctic access, and World Bank fallback trade/energy/mineral proxies. Key-gated sources such as high-resolution trade or energy feeds remain visibly labeled as gaps, not silently invented.",
    ),
]


def signal_methodology_html() -> str:
    """Render the explanatory layer that makes the five-signal frame auditable."""
    rationale = []
    for title, body in SIGNAL_SELECTION_RATIONALE:
        rationale.append(f'<div class="source-card"><h3>{html.escape(title)}</h3><p>{html.escape(body)}</p></div>')
    signal_rows = []
    for signal, (label, question, evidence) in SIGNAL_EXPLAINERS.items():
        signal_rows.append(
            "<tr>"
            f"<td><strong>{html.escape(label)}</strong><br><span class=\"tiny\">{html.escape(signal)}</span></td>"
            f"<td>{html.escape(question)}</td>"
            f"<td>{html.escape(evidence)}</td>"
            "</tr>"
        )
    return f"""<div class=\"card\"><h2>Approach: why the 5-signal fingerprint exists</h2><p class=\"muted\">This section explains the frame before the viewer sees country scores. WETD does not ask “is this country a war economy?” directly; it asks whether five observable allocation mechanisms are moving away from normal market allocation and toward national-security allocation.</p><div class=\"source-grid\">{''.join(rationale)}</div><details open><summary>Five signals, selection background, and data basis</summary><table><thead><tr><th>Signal</th><th>Why selected</th><th>Data basis in this batch</th></tr></thead><tbody>{''.join(signal_rows)}</tbody></table></details><p class=\"compact-note\">Practical use: start with the fingerprint, then open the country narrative to see whether the combination is a non-obvious inference or merely a list of obvious cause-effect facts.</p></div>"""

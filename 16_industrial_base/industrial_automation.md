# Industrial base: industrial automation
...
- 핵심 제약은 robots, PLCs, sensors, integration labor이다.
- 선행지표는 robot shipments, PLC backlogs, automation integrator capacity이다.
- 의존 도메인은 Robotics, manufacturing, logistics, and human capital이다.
...
|---|---|---|---|---|
| `industrial_automation_demand` | demand | index | higher_is_capacity | use_base |
| `industrial_automation_capacity` | capacity | index | higher_is_capacity | block |
| `industrial_automation_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(industrial_automation_capacity)
finance_score = normalized_inverse(industrial_automation_finance)
demand_score = normalized(industrial_automation_demand)
...
Baseline formula: `min(robot supply, controls supply, systems-integration labor)`.
...
- Base: min(robot supply, controls supply, systems-integration labor)에 의해 2035 정상 경로를 산출한다.

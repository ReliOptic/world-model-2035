# Healthcare systems: aging capacity
...
- 핵심 제약은 dependency ratios, chronic disease burden, long-term care workers이다.
- 선행지표는 old-age dependency, care vacancies, chronic disease prevalence이다.
- 의존 도메인은 Fiscal policy, labor markets, medicine, and insurance이다.
...
|---|---|---|---|---|
| `aging_capacity_demand` | demand | index | higher_is_capacity | use_base |
| `aging_capacity_capacity` | capacity | index | higher_is_capacity | block |
| `aging_capacity_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(aging_capacity_capacity)
finance_score = normalized_inverse(aging_capacity_finance)
demand_score = normalized(aging_capacity_demand)
...
Baseline formula: `min(care workforce, fiscal affordability, facility capacity, household affordability)`.
...
- Base: min(care workforce, fiscal affordability, facility capacity, household affordability)에 의해 2035 정상 경로를 산출한다.
...
- [clinical_trial_evolution.md](../11_science/medicine/clinical_trial_evolution.md)
- [automation_vs_augmentation.md](../08_economics/labor_markets/automation_vs_augmentation.md)
- [healthcare_workforce.md](../24_human_capital/healthcare_workforce.md)
...
- WHO World Health Statistics 2025, https://data.who.int/, 2026-05 확인.
- WHO UHC data, https://www.who.int/data/gho/data/themes/theme-details/GHO/universal-health-coverage, 2026-05 확인.

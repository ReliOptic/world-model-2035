# Financial markets: credit spreads
...
- 핵심 제약은 risk-free rates, default risk, liquidity, policy uncertainty이다.
- 선행지표는 IG/HY spreads, yield curve, default rates, bank lending standards이다.
- 의존 도메인은 CAPEX, construction, AI infrastructure, and fiscal policy이다.
...
|---|---|---|---|---|
| `credit_spreads_demand` | demand | index | higher_is_capacity | use_base |
| `credit_spreads_capacity` | capacity | index | higher_is_capacity | block |
| `credit_spreads_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(credit_spreads_capacity)
finance_score = normalized_inverse(credit_spreads_finance)
demand_score = normalized(credit_spreads_demand)
...
Baseline formula: `min(borrower cash flow, market liquidity, risk appetite, refinancing capacity)`.
...
- Base: min(borrower cash flow, market liquidity, risk appetite, refinancing capacity)에 의해 2035 정상 경로를 산출한다.

...
|---|---|---|---|---|
| 2026 | 7B-14B 모델이 2024년 플래그십 70B 수준 범용 성능을 공개 라이선스로 제공; 2B+ 파라미터 SLM이 스마트폰 NPU에서 실시간 실행 가능 단계 진입 | 증류 파이프라인이 오픈 생태계 표준으로 굳어지고 멀티모달 소형화가 동반 진행 | 저작권·학습데이터 규제로 오픈 라이선스 증류가 제한 | 85% |
| 2027 | 3B-7B 모델이 특정 수직(코딩, 수학, 의료, 법률)에서 프런티어 성능에 근접; 플래그십 스마트폰에 >2B 파라미터 SLM 기본 탑재가 업계 표준화 | 도메인 특화 증류가 엔터프라이즈 표준 배포 방식으로 정착 | 특화 SLM의 파인튜닝 비용이 예상보다 높고 유지보수 부담 커짐 | 78% |
| 2028 | 1B-3B 모델이 폰·PC 기본 탑재 수준의 범용 추론 능력 확보; 7B 모델 NPU 실시간 실행 보편화; 오픈소스 SLM이 대부분 범용 능력에서 프런티어 수준 90% 달성(오픈소스-프런티어 격차 빠르게 축소) | 플래그십 폰 NPU에서 7B 모델 실시간 실행이 보편화 | 플랫폼 사업자가 폐쇄형 SLM을 독점 배포해 오픈 생태계 분절 | 70% |
...
| 2032 | SLM-프런티어 격차는 "기술 한계"가 아니라 "데이터 접근과 증류 지연"의 문제로 재정의 | 프런티어 랩이 증류 친화적 모델을 정기 공개하는 체제가 정착 | 프런티어 랩이 증류를 차단하는 비공개 전략으로 전환 | 35% |
| 2033 | 대부분의 소비자·기업 업무는 3B-14B SLM과 에이전트 플랫폼의 조합으로 처리 | 범용 질의의 90%+가 SLM 처리로 이관 | 에이전트 오케스트레이션 복잡성이 단순 API 대비 총비용을 높임 | 30% |

# Built environment: permitting and labor bottlenecks
...
- 핵심 제약은 environmental review, zoning, grid interconnection, trades이다.
- 선행지표는 permit cycle time, queue length, journeyman vacancy rate이다.
- 의존 도메인은 Construction, grid, human capital, and public policy이다.
...
|---|---|---|---|---|
| `permitting_labor_bottlenecks_demand` | demand | index | higher_is_capacity | use_base |
| `permitting_labor_bottlenecks_capacity` | capacity | index | higher_is_capacity | block |
| `permitting_labor_bottlenecks_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(permitting_labor_bottlenecks_capacity)
finance_score = normalized_inverse(permitting_labor_bottlenecks_finance)
demand_score = normalized(permitting_labor_bottlenecks_demand)
...
Baseline formula: `min(permitting throughput, inspection capacity, labor availability)`.
...
- Base: min(permitting throughput, inspection capacity, labor availability)에 의해 2035 정상 경로를 산출한다.
...
- OECD Infrastructure, https://www.oecd.org/en/topics/infrastructure.html, 2026-05 확인.

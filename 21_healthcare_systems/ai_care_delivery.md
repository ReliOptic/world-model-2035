# Healthcare systems: AI care delivery
...
- 핵심 제약은 clinical workflow integration, liability, data quality, reimbursement이다.
- 선행지표는 FDA/EMA digital approvals, hospital AI pilots, claims adoption이다.
- 의존 도메인은 AI software, sensors, medicine, and cybersecurity이다.
...
|---|---|---|---|---|
| `ai_care_delivery_demand` | demand | index | higher_is_capacity | use_base |
| `ai_care_delivery_capacity` | capacity | index | higher_is_capacity | block |
| `ai_care_delivery_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(ai_care_delivery_capacity)
finance_score = normalized_inverse(ai_care_delivery_finance)
demand_score = normalized(ai_care_delivery_demand)
...
Baseline formula: `min(model reliability, workflow adoption, reimbursement, liability clarity)`.
...
- Base: min(model reliability, workflow adoption, reimbursement, liability clarity)에 의해 2035 정상 경로를 산출한다.

...
|---|---|---|---|---|
| 2026 | Willow 이후 임계값 이하 QEC 결과의 후속 검증과 실시간 디코딩 개선이 진행된다 | 논리 오류율 개선 속도가 예상을 웃돈다 | 후속 칩 공개가 지연된다 | 75% |
| 2027 | distance 확장과 GCP 연동 실험이 상용화 준비 지표가 된다 | 산업 파트너십이 화학·소재로 확대된다 | RCS 중심 성과라는 비판이 지속된다 | 65% |
| 2028 | 2029 실용 우위 목표를 앞두고 논리 오류율과 회로 깊이가 핵심 체크포인트가 된다 | 특정 양자화학 문제에서 HPC 대비 우위 신호가 나온다 | 오류율이 실용 알고리즘 요구치와 계속 벌어진다 | 55% |
| 2029 | Google은 첫 실용적 양자 우위 달성을 주장할 가능성이 있다 | 공개 검증 가능한 산업 문제에서 재현된다 | 목표가 2030년대 초반으로 미뤄진다 | 45% |
| 2030 | 양자 화학·소재 워크로드의 클라우드 파일럿이 확대된다 | GCP 통합이 개발자 채택을 가속한다 | IBM/Qiskit 생태계 대비 개발자층이 작다 | 55% |
| 2031 | 제한적 상업 서비스는 가능하지만 범용성은 낮다 | Alphabet 내부 AI+quantum 연계가 차별화된다 | 실용 워크로드 수가 제한된다 | 50% |
| 2032 | 논리 큐비트 확장이 사업성 판단의 중심이 된다 | 수백 논리큐비트 경로가 선명해진다 | 오류정정 오버헤드가 비용을 압박한다 | 45% |
| 2033 | Google은 양자 화학 서비스의 선두권으로 남는다 | 신약·배터리 문제에서 유료 고객이 늘어난다 | HPC/AI 대체기술이 우위를 흡수한다 | 55% |
| 2034 | 양자 서비스는 GCP 고급 연구 인프라로 자리잡는다 | 독립 수익 단위화 가능성이 커진다 | 연구비성 매출에 머문다 | 50% |
| 2035 | Google은 선두권 양자 클라우드 기업이지만 적용범위는 좁은 고가 워크로드 중심이다 | 실용 우위 문제가 복수 분야로 확장된다 | 2029 목표 지연 여파로 상용 신뢰가 약화된다 | 60% |

...
|---|---|---|---|---|
| 2026 | CoWoS 캐파 연간 70%+ 확대(5.5-레티클 인터포저 출하 진행 중), SoIC 기반 적층 AI 칩 출하, A16 위탁 양산 시작 | HBM4+CoWoS 일체형 공급이 예상보다 빠르게 안정화 | 수율 저하와 패키징 검사 난이도로 실효 출하 제한 | 88% |
| 2027 | A16 Super Power Rail 첫 상용 제품 출하, CoWoS 9.5-레티클 기술 검증(HBM 12개+AI 가속기 4개 통합 수준) | BSPDN 수율이 예상보다 빠르게 개선되며 고객 재수주 가속 | 배선 복잡도 증가로 A16 램프가 예상 대비 6-12개월 지연 | 55% |
| 2028 | CoWoS 14-레티클 양산 목표, 하이브리드 본딩이 HBM 이외 로직-로직 연결로 확산; 3D 패키징이 첨단 AI 가속기 BOM의 ~35-40% 차지 추정 | 3D 적층 표준화로 다이 설계 생산성 급상승 | 패키징 원가가 전체 BOM의 40%를 초과해 고객 채택 둔화 | 78% |
...
| 2031 | CFET 초기 파일럿 양산 (초고가 특수 애플리케이션); imec IRDS 로드맵 기준 3D VLSI CFET 목표 2034년 — 2031 파일럿은 수율·원가 모두 불확실 | CFET가 SRAM 셀 밀도 혁신을 이끌어 AI 가속기 캐시 용량 급상승 | CFET 수율 문제로 GAA 나노시트의 수명 연장 | 32% |
...
| 2034 | CFET 양산 확대 (특수 메모리·로직), 4nm 이하 설계의 표준 인터커넥트는 하이브리드 본딩 | 모놀리식 3D의 상용화가 메모리 밀도 한계 돌파 | 복잡도 대비 비용 효익 저하로 일부 응용은 이전 아키텍처 유지 | 28% |

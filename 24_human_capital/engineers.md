...
- 핵심 제약은 electrical, civil, semiconductor, software, nuclear engineers이다.
- 선행지표는 STEM graduates, job postings, wage premia, visa flows이다.
- 의존 도메인은 Semiconductors, energy, AI, defense, and construction이다.
...
|---|---|---|---|---|
| `engineers_demand` | demand | index | higher_is_capacity | use_base |
| `engineers_capacity` | capacity | index | higher_is_capacity | block |
| `engineers_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(engineers_capacity)
finance_score = normalized_inverse(engineers_finance)
demand_score = normalized(engineers_demand)
...
Baseline formula: `min(graduate pipeline, experienced engineer pool, mobility, retention)`.
...
- Base: min(graduate pipeline, experienced engineer pool, mobility, retention)에 의해 2035 정상 경로를 산출한다.

# Human capital: education and reskilling
...
- 핵심 제약은 adult learning access, skills mismatch, employer training이다.
- 선행지표는 training participation, PIAAC skills, online job-skill shifts이다.
- 의존 도메인은 Labor markets, AI, industrial automation, and policy이다.
...
|---|---|---|---|---|
| `education_reskilling_demand` | demand | index | higher_is_capacity | use_base |
| `education_reskilling_capacity` | capacity | index | higher_is_capacity | block |
| `education_reskilling_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(education_reskilling_capacity)
finance_score = normalized_inverse(education_reskilling_finance)
demand_score = normalized(education_reskilling_demand)
...
Baseline formula: `min(training access, relevance, worker time, employer adoption)`.
...
- Base: min(training access, relevance, worker time, employer adoption)에 의해 2035 정상 경로를 산출한다.

# Logistics and trade: supply-chain fragmentation
...
- 핵심 제약은 tariffs, export controls, sanctions, duplicated inventories이다.
- 선행지표는 trade restrictions, rerouting costs, inventory ratios, FDI shifts이다.
- 의존 도메인은 Geopolitics, semiconductors, critical minerals, and corporate roadmaps이다.
...
|---|---|---|---|---|
| `supply_chain_fragmentation_demand` | demand | index | higher_is_capacity | use_base |
| `supply_chain_fragmentation_capacity` | capacity | index | higher_is_capacity | block |
| `supply_chain_fragmentation_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(supply_chain_fragmentation_capacity)
finance_score = normalized_inverse(supply_chain_fragmentation_finance)
demand_score = normalized(supply_chain_fragmentation_demand)
...
Baseline formula: `min(open trade capacity, compliance capacity, redundant supply availability)`.
...
- Base: min(open trade capacity, compliance capacity, redundant supply availability)에 의해 2035 정상 경로를 산출한다.
...
- [semiconductor_self_reliance.md](../06_geopolitics/02_china/semiconductor_self_reliance.md)
- [bifurcated.md](../13_scenarios/bifurcated.md)
- [copper.md](../17_materials_and_mining/copper.md)
...
- World Bank Logistics Performance Index, https://lpi.worldbank.org/, 2026-05 확인.
- World Bank LPI 2023 release, https://www.worldbank.org/en/news/press-release/2023/04/21/world-bank-releases-logistics-performance-index-2023, 2026-05 확인.
- WTO World Trade Report 2025, https://www.wto.org/english/res_e/publications_e/wtr25_e.htm, 2026-05 확인.

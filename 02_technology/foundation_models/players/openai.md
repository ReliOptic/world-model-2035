...
|---|---|---|---|---|
| 2026 | GPT-6 학습 시작(Stargate Abilene), 자체 추론 칩 배포, o-시리즈 RL 패러다임 API 표준화 | Stargate 전력 램프업이 예정대로 되면 GPT-6 학습 일정 단축 | SMR 인허가·전력 인입 지연 시 GPT-6 출시 `1-2년` 밀림 | 82% |
| 2027 | GPT-6 출시 시나리오, System 1+2 통합 아키텍처 2세대, 에이전트 오케스트레이션 API 표준화 | GPT-6이 현 GPT-5 대비 `100x` 역량 주장 실현 | 규제(EU AI Act 고위험 분류) 또는 안전 평가 지연 | 78% |

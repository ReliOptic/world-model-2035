...
- 핵심 제약은 precision machine tools, CNC capacity, export controls이다.
- 선행지표는 machine-tool orders, high-end CNC utilization, control-system availability이다.
- 의존 도메인은 Aerospace, semiconductors, defense, and robotics이다.
...
|---|---|---|---|---|
| `machine_tools_demand` | demand | index | higher_is_capacity | use_base |
| `machine_tools_capacity` | capacity | index | higher_is_capacity | block |
| `machine_tools_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(machine_tools_capacity)
finance_score = normalized_inverse(machine_tools_finance)
demand_score = normalized(machine_tools_demand)
...
Baseline formula: `min(machine-tool availability, controller supply, qualified operator capacity)`.
...
- Base: min(machine-tool availability, controller supply, qualified operator capacity)에 의해 2035 정상 경로를 산출한다.

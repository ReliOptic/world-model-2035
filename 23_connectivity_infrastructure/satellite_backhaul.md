# Connectivity infrastructure: satellite backhaul
...
- 핵심 제약은 launch cadence, spectrum, ground terminals, orbital congestion이다.
- 선행지표는 satellite capacity, terminal cost, spectrum filings, launch cadence이다.
- 의존 도메인은 Space, defense, rural connectivity, and disaster resilience이다.
...
|---|---|---|---|---|
| `satellite_backhaul_demand` | demand | index | higher_is_capacity | use_base |
| `satellite_backhaul_capacity` | capacity | index | higher_is_capacity | block |
| `satellite_backhaul_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(satellite_backhaul_capacity)
finance_score = normalized_inverse(satellite_backhaul_finance)
demand_score = normalized(satellite_backhaul_demand)
...
Baseline formula: `min(satellite capacity, terminal supply, ground network, spectrum access)`.
...
- Base: min(satellite capacity, terminal supply, ground network, spectrum access)에 의해 2035 정상 경로를 산출한다.
...
- [network.md](../02_technology/compute_infrastructure/network.md)
- [starlink.md](../05_space/spacex/starlink.md)
- [critical_infra.md](../07_defense/cyber/critical_infra.md)
...
- ITU Facts and Figures 2025, https://www.itu.int/ff2025, 2026-05 확인.
- ITU Measuring Digital Development reports, https://www.itu.int/itu-d/reports/statistics/facts-figures-2025/, 2026-05 확인.

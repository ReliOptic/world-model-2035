# Logistics and trade: rail and trucking
...
- 핵심 제약은 drivers, rail slots, equipment, fuel prices이다.
- 선행지표는 truckload rates, rail velocity, driver vacancies, diesel prices이다.
- 의존 도메인은 Ports, agriculture, construction, and industrial base이다.
...
|---|---|---|---|---|
| `rail_trucking_demand` | demand | index | higher_is_capacity | use_base |
| `rail_trucking_capacity` | capacity | index | higher_is_capacity | block |
| `rail_trucking_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(rail_trucking_capacity)
finance_score = normalized_inverse(rail_trucking_finance)
demand_score = normalized(rail_trucking_demand)
...
Baseline formula: `min(driver availability, rail capacity, equipment availability, fuel affordability)`.
...
- Base: min(driver availability, rail capacity, equipment availability, fuel affordability)에 의해 2035 정상 경로를 산출한다.

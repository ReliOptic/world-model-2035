# Healthcare systems: insurance costs
...
- 핵심 제약은 medical inflation, payer mix, aging, catastrophic risk이다.
- 선행지표는 premiums, out-of-pocket burden, claims inflation, UHC coverage이다.
- 의존 도메인은 Financial markets, fiscal policy, aging, and climate risk이다.
...
|---|---|---|---|---|
| `insurance_costs_demand` | demand | index | higher_is_capacity | use_base |
| `insurance_costs_capacity` | capacity | index | higher_is_capacity | block |
| `insurance_costs_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(insurance_costs_capacity)
finance_score = normalized_inverse(insurance_costs_finance)
demand_score = normalized(insurance_costs_demand)
...
Baseline formula: `min(payer capacity, household affordability, fiscal support, risk pooling)`.
...
- Base: min(payer capacity, household affordability, fiscal support, risk pooling)에 의해 2035 정상 경로를 산출한다.

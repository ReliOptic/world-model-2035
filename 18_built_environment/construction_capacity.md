# Built environment: construction capacity
...
- 핵심 제약은 EPC capacity, skilled trades, materials, permitting이다.
- 선행지표는 construction starts, backlog, wage premia, permit delays이다.
- 의존 도메인은 Industrial base, grids, financial markets, and human capital이다.
...
|---|---|---|---|---|
| `construction_capacity_demand` | demand | index | higher_is_capacity | use_base |
| `construction_capacity_capacity` | capacity | index | higher_is_capacity | block |
| `construction_capacity_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(construction_capacity_capacity)
finance_score = normalized_inverse(construction_capacity_finance)
demand_score = normalized(construction_capacity_demand)
...
Baseline formula: `min(EPC capacity, materials availability, labor supply, financing availability)`.
...
- Base: min(EPC capacity, materials availability, labor supply, financing availability)에 의해 2035 정상 경로를 산출한다.

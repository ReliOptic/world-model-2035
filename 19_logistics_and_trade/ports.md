...
- 핵심 제약은 berths, cranes, dredging, customs systems이다.
- 선행지표는 dwell time, TEU throughput, crane productivity, capex이다.
- 의존 도메인은 Shipping, rail, trucking, food, and minerals이다.
...
|---|---|---|---|---|
| `ports_demand` | demand | index | higher_is_capacity | use_base |
| `ports_capacity` | capacity | index | higher_is_capacity | block |
| `ports_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(ports_capacity)
finance_score = normalized_inverse(ports_finance)
demand_score = normalized(ports_demand)
...
Baseline formula: `min(berth capacity, yard capacity, hinterland capacity, customs throughput)`.
...
- Base: min(berth capacity, yard capacity, hinterland capacity, customs throughput)에 의해 2035 정상 경로를 산출한다.

# Circular economy: plastics waste
...
- 핵심 제약은 collection, recycling economics, regulation, petrochemical spreads이다.
- 선행지표는 plastic waste flows, recycled resin prices, EPR laws, treaty progress이다.
- 의존 도메인은 Petrochemicals, consumer goods, logistics, and climate policy이다.
...
|---|---|---|---|---|
| `plastics_waste_demand` | demand | index | higher_is_capacity | use_base |
| `plastics_waste_capacity` | capacity | index | higher_is_capacity | block |
| `plastics_waste_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(plastics_waste_capacity)
finance_score = normalized_inverse(plastics_waste_finance)
demand_score = normalized(plastics_waste_demand)
...
Baseline formula: `min(collection, sorting, reprocessing, demand for recycled resin)`.
...
- Base: min(collection, sorting, reprocessing, demand for recycled resin)에 의해 2035 정상 경로를 산출한다.

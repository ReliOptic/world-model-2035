...
|---|---|---|---|---|
| 2026 | 플래그십 폰에서 3B SLM 실시간 추론 시 배터리 `2-4시간` 지속 가능; 스마트폰 AI 칩 ~50 TOPS; 배터리·열 구조 제약 지속(80%+ 확률) | 저정밀 양자화로 `+50%` 지속시간 확보 | 열 쓰로틀링으로 광고 성능 대비 실사용 격차 확대 | 82% |
| 2027 | `300 Wh/kg` 급 배터리가 플래그십 폰에 부분 도입; 온디바이스 7B 파라미터 실시간 추론 플래그십 표준 | 글래스 배터리 지속시간 `3-4시간`으로 개선 | 배터리 원가·안전성 이슈로 상용 지연 | 79% |
| 2028 | 온디바이스 LLM 추론의 에너지 비용이 토큰당 `~100 mJ` 수준으로 하락; NPU ~200 TOPS 스마트폰 표준; 신규 노트북 80%+ NPU 탑재 | 토큰당 에너지 연 `2배` 개선 추세가 유지 | 모델 크기 확장이 효율 개선을 상쇄 | 65% |
| 2029 | 폰 플래그십이 하루 평균 `2-3시간` 연속 AI 사용에도 배터리 유지; 배터리·열 구조 제약 지속(80%+ 확률) | 저전력 AI 보조 프로세서(ULP NPU)가 상시 기능 처리 | 상시 기능 사용 확대로 평균 배터리 수명 오히려 단축 | 82% |
| 2030 | 실리콘-탄소 음극, 리튬 메탈 솔리드스테이트 초기 제품이 프리미엄 디바이스 진입; NPU 200+ TOPS 표준; 30B+ 파라미터 온디바이스 추론 가능 | `400-500 Wh/kg` 솔리드스테이트 초기 상용화 | 양산 수율·안전 이슈로 틈새 제품 한정 | 37% |
...
| 2034 | 주요 소비자 기기의 AI 상시 기능이 배터리 예산을 정상 할당 영역에 포함; 배터리·열 구조 제약 지속(80%+ 확률) | OS가 에너지 예산을 워크로드별 자동 라우팅 | 사용자 행태 변화로 에너지 수요 재급증 | 82% |
| 2035 | 온디바이스 AI는 배터리-열 예산 내에서 "상시 3B-7B 모델 + 간헐적 14B 모델" 체제로 정착; 배터리·열 한계 구조적 지속(80%+ 확률) | 배터리·메모리·냉각 공동 혁신으로 체급 상한 확장 | 혁신 없이 틈새 용도 한정 | 27% |

...
- 핵심 제약은 brine/hard-rock capacity, refining, battery-grade quality이다.
- 선행지표는 lithium price, project FID, conversion capacity이다.
- 의존 도메인은 Batteries, EVs, ESS, and critical minerals이다.
...
|---|---|---|---|---|
| `lithium_demand` | demand | index | higher_is_capacity | use_base |
| `lithium_capacity` | capacity | index | higher_is_capacity | block |
| `lithium_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(lithium_capacity)
finance_score = normalized_inverse(lithium_finance)
demand_score = normalized(lithium_demand)
...
Baseline formula: `min(mined lithium, conversion capacity, qualified battery-grade supply)`.
...
- Base: min(mined lithium, conversion capacity, qualified battery-grade supply)에 의해 2035 정상 경로를 산출한다.

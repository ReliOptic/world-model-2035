...
- 핵심 제약은 collection, informal processing, traceability, recovery economics이다.
- 선행지표는 e-waste generation, documented recycling rate, precious metal prices이다.
- 의존 도메인은 Semiconductors, connectivity, consumer electronics, and regulation이다.
...
|---|---|---|---|---|
| `e_waste_demand` | demand | index | higher_is_capacity | use_base |
| `e_waste_capacity` | capacity | index | higher_is_capacity | block |
| `e_waste_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(e_waste_capacity)
finance_score = normalized_inverse(e_waste_finance)
demand_score = normalized(e_waste_demand)
...
Baseline formula: `min(collection, formal processing, metal recovery, compliance enforcement)`.
...
- Base: min(collection, formal processing, metal recovery, compliance enforcement)에 의해 2035 정상 경로를 산출한다.

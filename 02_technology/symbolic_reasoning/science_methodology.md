...
|---|---|---|---|---|
| 2026 | Nature/Science/Cell이 AI 기반 연구에 대한 공식 재현성·검증 가이드라인 채택(Royal Society 2024-05 경고 이후 제도화); AI-driven 논문 인용 비중 주요 저널 20%+ | 가이드라인 채택이 AI-driven 과학의 신뢰 회복 전환점이 됨 | 대형 AI 기반 retraction이 1건+ 발생하면 체계적 불신 증가 | 82% |
| 2027 | A-Lab 계열 자율 실험실이 재료·단백질·소분자 3개 분야에서 동시 운용; 자율 실험 처리량이 종래 대비 10배+(월 100개+ 후보 합성·스크리닝) | 자율 실험실 네트워크가 글로벌로 연결되어 실험 처리량이 종래 대비 10배+ | 물리 합성·실험 단계가 AI 예측 속도를 따라가지 못해 백로그 축적 | 76% |
| 2028 | AI 발견이 인간 해석 능력을 초과하는 영역(고차원 상호작용, 대규모 시뮬)이 주요 저널에 명확히 드러남; GNoME 38만 안정 결정구조 후보 대비 실험 검증률 <1%로 구조적 backlog 수만 건 축적 | 해석가능성 연구가 circuit-level 이해와 도메인 과학을 결합해 black-box를 부분 완화 | SAE·circuit tracing이 실용 규모로 확장되지 못해 black-box 고착 | 55% |
| 2029 | 노벨상급 발견 중 최소 1건이 AI-assisted임이 공식 인정(2024 노벨화학상 AlphaFold 2 기여 인정 선례 기반, 후속 사례 가능) | peer-review가 AI-assisted를 별도 평가하는 체계를 갖추어 신뢰 보존 | 저자권·기여도·책임 소재 혼란이 학계 거버넌스 위기 | 30% |

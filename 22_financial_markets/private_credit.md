...
- 핵심 제약은 fundraising, covenant quality, refinancing walls, opacity이다.
- 선행지표는 private credit AUM, spreads, defaults, fundraising이다.
- 의존 도메인은 Infrastructure, real estate, energy transition, and corporate roadmaps이다.
...
|---|---|---|---|---|
| `private_credit_demand` | demand | index | higher_is_capacity | use_base |
| `private_credit_capacity` | capacity | index | higher_is_capacity | block |
| `private_credit_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(private_credit_capacity)
finance_score = normalized_inverse(private_credit_finance)
demand_score = normalized(private_credit_demand)
...
Baseline formula: `min(fund dry powder, underwriting quality, exit liquidity, borrower demand)`.
...
- Base: min(fund dry powder, underwriting quality, exit liquidity, borrower demand)에 의해 2035 정상 경로를 산출한다.

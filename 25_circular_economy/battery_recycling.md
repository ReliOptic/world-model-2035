...
- 핵심 제약은 collection, black mass processing, chemistry mix, regulation이다.
- 선행지표는 retired battery volume, recycling capacity, recovered metal prices이다.
- 의존 도메인은 Batteries, EVs, critical minerals, and waste regulation이다.
...
|---|---|---|---|---|
| `battery_recycling_demand` | demand | index | higher_is_capacity | use_base |
| `battery_recycling_capacity` | capacity | index | higher_is_capacity | block |
| `battery_recycling_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(battery_recycling_capacity)
finance_score = normalized_inverse(battery_recycling_finance)
demand_score = normalized(battery_recycling_demand)
...
Baseline formula: `min(collection rate, processing capacity, recovery yield, buyer qualification)`.
...
- Base: min(collection rate, processing capacity, recovery yield, buyer qualification)에 의해 2035 정상 경로를 산출한다.

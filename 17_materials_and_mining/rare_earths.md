# Materials and mining: rare earths
...
- 핵심 제약은 mining concentration, separation capacity, magnet supply이다.
- 선행지표는 oxide prices, separation permits, magnet export rules이다.
- 의존 도메인은 Wind, EV motors, defense, and robotics이다.
...
|---|---|---|---|---|
| `rare_earths_demand` | demand | index | higher_is_capacity | use_base |
| `rare_earths_capacity` | capacity | index | higher_is_capacity | block |
| `rare_earths_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(rare_earths_capacity)
finance_score = normalized_inverse(rare_earths_finance)
demand_score = normalized(rare_earths_demand)
...
Baseline formula: `min(oxide supply, separation capacity, magnet production capacity)`.
...
- Base: min(oxide supply, separation capacity, magnet production capacity)에 의해 2035 정상 경로를 산출한다.

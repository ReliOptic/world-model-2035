# Healthcare systems: hospital capacity
...
- 핵심 제약은 beds, clinicians, equipment, surge capacity이다.
- 선행지표는 bed occupancy, wait times, nurse vacancies, capex이다.
- 의존 도메인은 Health workforce, fiscal policy, AI care delivery, and biosecurity이다.
...
|---|---|---|---|---|
| `hospital_capacity_demand` | demand | index | higher_is_capacity | use_base |
| `hospital_capacity_capacity` | capacity | index | higher_is_capacity | block |
| `hospital_capacity_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(hospital_capacity_capacity)
finance_score = normalized_inverse(hospital_capacity_finance)
demand_score = normalized(hospital_capacity_demand)
...
Baseline formula: `min(beds, clinicians, equipment, operating budget)`.
...
- Base: min(beds, clinicians, equipment, operating budget)에 의해 2035 정상 경로를 산출한다.

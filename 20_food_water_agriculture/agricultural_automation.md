# Food, water, and agriculture: agricultural automation
...
- 핵심 제약은 farm labor, sensors, robotics, connectivity, capital cost이다.
- 선행지표는 autonomous equipment sales, precision ag adoption, rural broadband이다.
- 의존 도메인은 Connectivity, sensors, robotics, and human capital이다.
...
|---|---|---|---|---|
| `agricultural_automation_demand` | demand | index | higher_is_capacity | use_base |
| `agricultural_automation_capacity` | capacity | index | higher_is_capacity | block |
| `agricultural_automation_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(agricultural_automation_capacity)
finance_score = normalized_inverse(agricultural_automation_finance)
demand_score = normalized(agricultural_automation_demand)
...
Baseline formula: `min(robotics maturity, farm ROI, connectivity, service-network capacity)`.
...
- Base: min(robotics maturity, farm ROI, connectivity, service-network capacity)에 의해 2035 정상 경로를 산출한다.

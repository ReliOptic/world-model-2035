# Materials and mining: steel, cement, and chemicals
...
- 핵심 제약은 blast furnace/electric arc transition, clinker capacity, chemical feedstocks이다.
- 선행지표는 steel spreads, cement shipments, chemical margins이다.
- 의존 도메인은 Construction, industrial base, energy, and logistics이다.
...
|---|---|---|---|---|
| `steel_cement_chemicals_demand` | demand | index | higher_is_capacity | use_base |
| `steel_cement_chemicals_capacity` | capacity | index | higher_is_capacity | block |
| `steel_cement_chemicals_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(steel_cement_chemicals_capacity)
finance_score = normalized_inverse(steel_cement_chemicals_finance)
demand_score = normalized(steel_cement_chemicals_demand)
...
Baseline formula: `min(material production, decarbonization compliance, transport availability)`.
...
- Base: min(material production, decarbonization compliance, transport availability)에 의해 2035 정상 경로를 산출한다.
...
- [storage.md](../03_energy/renewables/storage.md)
...
- IEA Critical Minerals topic, https://www.iea.org/topics/critical-minerals, 2026-05 확인.
- USGS Mineral Commodity Summaries, https://www.usgs.gov/centers/national-minerals-information-center/mineral-commodity-summaries, 2026-05 확인.

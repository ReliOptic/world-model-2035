...
|---|---|---|---|---|
| 2026 | Copilot+ 40 TOPS가 PC 사실상 표준, 폰 플래그십은 3B-8B SLM 기본 탑재; 스마트폰 AI 칩 ~50 TOPS(Apple A19/Qualcomm X2 Elite) | 중급 폰까지 3B SLM이 확산 | NPU 활용 앱 부족으로 기능 잠재력 정체 | 88% |
| 2027 | 80-120 TOPS NPU 구간 진입, 멀티모달 온디바이스 실시간 추론 보편화; 플래그십 폰 온디바이스 AI 표준 기능(88%+ 확률) | 온디바이스 비디오 이해가 기본 기능 | 메모리 대역폭 제약으로 큰 모델 지연 문제 | 80% |
| 2028 | 폰·PC·웨어러블 간 모델 상태 공유가 표준 API로 제공; 신규 노트북 80%+ NPU 탑재; 온디바이스 7B 파라미터 실시간 추론 표준; 온디바이스 AI 모든 플래그십 폰 표준 기능(92%+ 확률) | 연속적 컨텍스트 공유 "개인 에이전트" 정착 | 플랫폼별 독자 표준으로 파편화 | 65% |
| 2029 | 온디바이스 14B 모델 실시간 실행이 플래그십 폰에서 가능; NPU ~200 TOPS | 모바일 NPU가 PC 수준 TOPS에 근접 | 배터리·열 제약으로 일상 사용은 7B 상한 유지 | 55% |
| 2030 | 주요 소비자 AI 상호작용의 60%+가 온디바이스에서 완결; NPU 200+ TOPS; 온디바이스 30B+ 파라미터 추론 가능 | 클라우드는 장문 추론·에이전트 오케스트레이션에 특화 | 프라이버시·데이터 주권 규제로 클라우드 연동 복잡화 | 60% |
| 2031 | 온디바이스 모델 업데이트가 앱 스토어처럼 OTA 표준화; 온디바이스 AI 모든 플래그십 폰 기본 계층(92% 확률) | 모델 마켓플레이스 활성화 | 보안·무결성 이슈로 업데이트 검증 비용 증가 | 78% |
| 2032 | 웨어러블(글래스·링·이어버드)에서도 1B-3B 모델 상시 실행 | 저전력 NPU 세대교체 가속 | 폼팩터 제약으로 전용 모델 분화 | 30% |
...
| 2035 | 온디바이스 AI는 "기능"이 아니라 "OS의 기본 계층"이 되며, 폰·PC·웨어러블·자동차에서 일관 경험 제공; 배터리·열 구조 제약 지속(80%+ 확률) | 다기기 에이전트 경험이 규격 표준에 도달 | 독점 생태계 경쟁으로 사용자 고착화 심화 | 25% |
...
- [./aui.md](./aui.md)
- [../foundation_models/slm_roadmap.md](../foundation_models/slm_roadmap.md)

...
- 핵심 제약은 factory throughput, supplier qualification, machine availability이다.
- 선행지표는 industrial production index, PMI, order backlog이다.
- 의존 도메인은 Semiconductors, defense, construction, and energy equipment이다.
...
|---|---|---|---|---|
| `manufacturing_capacity_demand` | demand | index | higher_is_capacity | use_base |
| `manufacturing_capacity_capacity` | capacity | index | higher_is_capacity | block |
| `manufacturing_capacity_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(manufacturing_capacity_capacity)
finance_score = normalized_inverse(manufacturing_capacity_finance)
demand_score = normalized(manufacturing_capacity_demand)
...
Baseline formula: `min(factory utilization headroom, supplier capacity, skilled labor availability)`.
...
- Base: min(factory utilization headroom, supplier capacity, skilled labor availability)에 의해 2035 정상 경로를 산출한다.

...
|---|---|---|---|---|
| 2026 | Llama 4 Behemoth(총 2T 파라미터, 활성 288B) 공개 출시, Llama 5 아키텍처 발표, EU 배포 제한 해소 협상; Llama 계열 오픈소스 다운로드 누적 10억 회+ | Behemoth 출시가 오픈 가중치 최강 모델 포지션 굳힘 | EU AI Act 고위험 분류로 배포 제한 장기화 | 80% |
| 2027 | Llama 5 — 추론 통합(Think 모드), 코딩 에이전트, `10M+` 컨텍스트 표준화; 오픈소스 모델이 현재 GPT-5 수준 90% 성능을 무료로 제공 | 오픈 가중치 MoE 생태계가 API 기반 경쟁사를 가격으로 압도 | 클로즈드 모델 역량 격차가 재확대 시 오픈 전략 매력 감소 | 78% |
| 2028 | Meta AI 어시스턴트가 `35억+ MAU` 플랫폼에서 일상 업무 자동화 핵심 도구; Fortune 500 기업 50%+가 Llama 기반 사내 에이전트 배포 | 소셜 플랫폼 에이전트가 광고 수익 이외의 서비스 수익 축 형성 | 소셜 플랫폼 AI 사용 규제(EU DSA·미국 아동보호법)로 기능 제한 | 60% |

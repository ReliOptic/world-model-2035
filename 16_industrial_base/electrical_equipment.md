# Industrial base: electrical equipment
...
- 핵심 제약은 transformers, switchgear, HVDC components, cable capacity이다.
- 선행지표는 transformer lead times, grid CAPEX, utility interconnection queues이다.
- 의존 도메인은 Power grid, data centers, renewables, and industrial automation이다.
...
|---|---|---|---|---|
| `electrical_equipment_demand` | demand | index | higher_is_capacity | use_base |
| `electrical_equipment_capacity` | capacity | index | higher_is_capacity | block |
| `electrical_equipment_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(electrical_equipment_capacity)
finance_score = normalized_inverse(electrical_equipment_finance)
demand_score = normalized(electrical_equipment_demand)
...
Baseline formula: `min(transformer capacity, switchgear capacity, cable capacity, permitting throughput)`.
...
- Base: min(transformer capacity, switchgear capacity, cable capacity, permitting throughput)에 의해 2035 정상 경로를 산출한다.
...
- [drone_swarms.md](../07_defense/autonomous_weapons/drone_swarms.md)
...
- UNIDO Industrial Development Report 2026, https://www.unido.org/publications/industrial-development-report-series, 2026-05 확인.
- UNIDO World Manufacturing Production, https://stat.unido.org/, 2026-05 확인.

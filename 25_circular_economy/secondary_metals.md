# Circular economy: secondary metals
...
- 핵심 제약은 scrap quality, sorting, energy, buyer standards이다.
- 선행지표는 scrap flows, secondary metal spreads, sorting capacity이다.
- 의존 도메인은 Critical minerals, steel, construction, and industrial base이다.
...
|---|---|---|---|---|
| `secondary_metals_demand` | demand | index | higher_is_capacity | use_base |
| `secondary_metals_capacity` | capacity | index | higher_is_capacity | block |
| `secondary_metals_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(secondary_metals_capacity)
finance_score = normalized_inverse(secondary_metals_finance)
demand_score = normalized(secondary_metals_demand)
...
Baseline formula: `min(scrap availability, sorting quality, smelting capacity, buyer acceptance)`.
...
- Base: min(scrap availability, sorting quality, smelting capacity, buyer acceptance)에 의해 2035 정상 경로를 산출한다.
...
- [lithium.md](../17_materials_and_mining/lithium.md)
- [battery_roadmap.md](../04_vehicles/ev/battery_roadmap.md)
- [nature_tech_convergence.md](../01_climate_and_nature/nature_tech_convergence.md)
...
- UNEP Global Waste Management Outlook 2024, https://wedocs.unep.org/handle/20.500.11822/44939, 2026-05 확인.
- ITU Global E-waste Monitor 2024, https://www.itu.int/en/ITU-D/Environment/Pages/Publications/The-Global-E-waste-Monitor-2024.aspx, 2026-05 확인.

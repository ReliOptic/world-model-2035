# Built environment: housing and real estate
...
- 핵심 제약은 rates, land, household formation, insurance이다.
- 선행지표는 housing starts, mortgage rates, vacancy, insurance costs이다.
- 의존 도메인은 Financial markets, climate, labor mobility, and infrastructure이다.
...
|---|---|---|---|---|
| `housing_real_estate_demand` | demand | index | higher_is_capacity | use_base |
| `housing_real_estate_capacity` | capacity | index | higher_is_capacity | block |
| `housing_real_estate_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(housing_real_estate_capacity)
finance_score = normalized_inverse(housing_real_estate_finance)
demand_score = normalized(housing_real_estate_demand)
...
Baseline formula: `min(household demand, affordability, construction capacity, insurability)`.
...
- Base: min(household demand, affordability, construction capacity, insurability)에 의해 2035 정상 경로를 산출한다.

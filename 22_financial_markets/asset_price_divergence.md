# Financial markets: asset price divergence
...
- 핵심 제약은 earnings expectations, discount rates, liquidity, narrative intensity이다.
- 선행지표는 multiples, flows, implied volatility, credit spreads이다.
- 의존 도메인은 Opportunity intelligence, AI, energy, and macro scenarios이다.
...
|---|---|---|---|---|
| `asset_price_divergence_demand` | demand | index | higher_is_capacity | use_base |
| `asset_price_divergence_capacity` | capacity | index | higher_is_capacity | block |
| `asset_price_divergence_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(asset_price_divergence_capacity)
finance_score = normalized_inverse(asset_price_divergence_finance)
demand_score = normalized(asset_price_divergence_demand)
...
Baseline formula: `current implied expectation minus deterministic fundamental line`.
...
- Base: current implied expectation minus deterministic fundamental line에 의해 2035 정상 경로를 산출한다.

# Connectivity infrastructure: fiber backbone
...
- 핵심 제약은 fiber routes, rights-of-way, optical equipment, power이다.
- 선행지표는 fiber route miles, lit capacity, optical backlog, capex이다.
- 의존 도메인은 Cloud, data centers, telecom, and urban infrastructure이다.
...
|---|---|---|---|---|
| `fiber_backbone_demand` | demand | index | higher_is_capacity | use_base |
| `fiber_backbone_capacity` | capacity | index | higher_is_capacity | block |
| `fiber_backbone_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(fiber_backbone_capacity)
finance_score = normalized_inverse(fiber_backbone_finance)
demand_score = normalized(fiber_backbone_demand)
...
Baseline formula: `min(fiber construction, optical equipment, rights-of-way, power availability)`.
...
- Base: min(fiber construction, optical equipment, rights-of-way, power availability)에 의해 2035 정상 경로를 산출한다.

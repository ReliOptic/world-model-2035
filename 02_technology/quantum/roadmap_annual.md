...
|---|---|---|---|---|
| 2026 | IBM Kookaburra qLDPC 모듈 출시, Google Willow 후속 오류율 개선 발표, Origin Wukong 클라우드 확장 | qLDPC 메모리 실질 논리 큐비트 수명이 설계치에 근접 | Kookaburra 수율 저하로 멀티칩 결합 시연 지연 | 65% |
| 2027 | IBM Cockatoo: 두 QEC 모듈 간 얽힘 시연. Google distance-9+ 코드 시연 | 논리 큐비트 간 게이트 충실도 99.9% 달성 | 모듈 간 결합 오류율이 단일 모듈 오류율을 초과해 확장 이점 상쇄 | 55% |

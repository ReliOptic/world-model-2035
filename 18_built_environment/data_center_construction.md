# Built environment: data center construction
...
- 핵심 제약은 power-ready sites, cooling, transformers, construction labor이다.
- 선행지표는 leased MW, powered shell delivery, interconnection queues이다.
- 의존 도메인은 AI compute, grid, semiconductors, and finance이다.
...
|---|---|---|---|---|
| `data_center_construction_demand` | demand | index | higher_is_capacity | use_base |
| `data_center_construction_capacity` | capacity | index | higher_is_capacity | block |
| `data_center_construction_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(data_center_construction_capacity)
finance_score = normalized_inverse(data_center_construction_finance)
demand_score = normalized(data_center_construction_demand)
...
Baseline formula: `min(power interconnect, shell delivery, cooling supply, GPU installation cadence)`.
...
- Base: min(power interconnect, shell delivery, cooling supply, GPU installation cadence)에 의해 2035 정상 경로를 산출한다.

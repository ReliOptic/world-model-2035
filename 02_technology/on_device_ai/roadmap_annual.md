...
| 2027 | 80-120 TOPS NPU 구간 진입, 멀티모달 온디바이스 실시간 추론 보편화 | 온디바이스 비디오 이해가 기본 기능 | 메모리 대역폭 제약으로 큰 모델 지연 문제 | 80% |
...
| 2031 | 온디바이스 모델 업데이트가 앱 스토어처럼 OTA 표준화 | 모델 마켓플레이스 활성화 | 보안·무결성 이슈로 업데이트 검증 비용 증가 | 35% |

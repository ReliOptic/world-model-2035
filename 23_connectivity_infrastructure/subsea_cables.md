# Connectivity infrastructure: subsea cables
...
- 핵심 제약은 cable ships, landing permits, geopolitics, repair capacity이다.
- 선행지표는 cable RFS dates, ship availability, outages, landing licenses이다.
- 의존 도메인은 Cloud, geopolitics, defense, and trade이다.
...
|---|---|---|---|---|
| `subsea_cables_demand` | demand | index | higher_is_capacity | use_base |
| `subsea_cables_capacity` | capacity | index | higher_is_capacity | block |
| `subsea_cables_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(subsea_cables_capacity)
finance_score = normalized_inverse(subsea_cables_finance)
demand_score = normalized(subsea_cables_demand)
...
Baseline formula: `min(cable manufacturing, installation ships, landing permits, repair capacity)`.
...
- Base: min(cable manufacturing, installation ships, landing permits, repair capacity)에 의해 2035 정상 경로를 산출한다.

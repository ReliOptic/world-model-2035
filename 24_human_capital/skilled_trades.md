...
- 핵심 제약은 electricians, welders, construction trades, licensing throughput이다.
- 선행지표는 vacancies, wages, apprenticeship completion, retirements이다.
- 의존 도메인은 Construction, grid, industrial base, and logistics이다.
...
|---|---|---|---|---|
| `skilled_trades_demand` | demand | index | higher_is_capacity | use_base |
| `skilled_trades_capacity` | capacity | index | higher_is_capacity | block |
| `skilled_trades_finance` | finance | index | higher_is_constraint | use_base |

## 2035 기준선 산출 방식

```text
capacity_score = normalized(skilled_trades_capacity)
finance_score = normalized_inverse(skilled_trades_finance)
demand_score = normalized(skilled_trades_demand)
...
Baseline formula: `min(training throughput, retention, immigration, automation substitution)`.
...
- Base: min(training throughput, retention, immigration, automation substitution)에 의해 2035 정상 경로를 산출한다.
